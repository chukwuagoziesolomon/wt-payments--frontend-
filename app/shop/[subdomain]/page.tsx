...
export default function StorefrontPage() {
        ...
        if (!cancelled)
          setProducts(
            Array.isArray(json.data.products)
              ? json.data.products.map(normalizeProduct)
              : []
          );
      } catch {
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#0e0e10] px-4">
        <div className="text-center">
          ...
          <p className="text-white/40 text-sm">
            {error || "This storefront does not exist or has been removed."}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div
      ...
      style={
        { "--primary": primary, "--accent": accent } as React.CSSProperties
      }
    >
      {/* Banner */}
      <div
        ...
        style={{
          background: shop.banner_url
            ? undefined
            : `linear-gradient(135deg, ${primary}40, ${accent}25)`,
        }}
      >
        {shop.banner_url && (
          <img
      ...
      {cartOpen && (
        ...
        >
          <div
            className="absolute right-0 top-0 h-full w-full max-w-md bg-[#111119] border-l border-white/[0.08] shadow-2xl flex flex-col"
              ...
              {cartItems.length === 0 ? (
                ...
                <div className="space-y-4">
                  {cartItems.map((item) => (
                    <div
                      key={item.id}
                      className="flex gap-4 p-3 rounded-xl bg-[#19191d] border border-white/[0.06]"
                    >
