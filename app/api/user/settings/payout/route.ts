...
export async function GET(req: Request) {
  ...
  try {
    ...
    const backendUrl = `${apiBase}/api/user/settings/payout${url.search}`;
    ...
    return NextResponse.json({ error: true, data: "Failed to fetch payout settings" }, { status: 502 });
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const res = await fetch(`${apiBase}/api/user/settings/payout`, {
      method: "POST",
      headers,
      body,
      cache: "no-store",
    ...
    return NextResponse.json({ error: true, data: "Failed to update payout settings" }, { status: 502 });
  }
}
